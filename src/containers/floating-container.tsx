...
export interface FloatingContainerProps
  extends ComponentProps,
    Pick<AutoPlacementOptions, "alignment" | "allowedPlacements">,
...
export default function FloatingContainer({
  ...
  const reference: HTMLElement =
    (floatAround &&
      ("current" in floatAround
        ? floatAround.current?.ref?.current || floatAround.current
        : floatAround)) ||
    document.body;

  const onOpenChange = useCallback(
    (inOpen: boolean, event: Event, reason?: string) => {
  ...
  const selfUpdate = useCallback(
    (update: { open?: boolean | "toggle" }, echo?: boolean) => {
