...
function ScrollXNode({
  ...
  const updateScroll = useCallback((update: { position?: number; percentage?: number; resize?: boolean }) => {
  ...
  const handleDrag = useCallback((e: MouseEvent | TouchEvent) => {
    e.preventDefault();
    e.stopPropagation();
    const posValue = (e as MouseEvent).clientX || (e as TouchEvent).touches ? (e as TouchEvent).touches[0].clientX : 0;
  ...
  const handleDragStart = useCallback((e: MouseEvent | TouchEvent) => {
    e.preventDefault();
    e.stopPropagation();
    const initialPosValue = (e as MouseEvent).clientX || (e as TouchEvent).touches ? (e as TouchEvent).touches[0].clientX : 0;
  ...
  const sbc: string | string[] | string[][] | (string | string[])[] = ['cursor-pointer', scrollBarClasses];
  const fc = (input: string[] | string | string[][] | (string | string[])[]) => splitAndFlat([input], ' ').join(' ');
